...
def login():
    return flask_portal.auth.auth0.authorize_redirect(redirect_uri=Auth.AUTH0_CALLBACK_URL, audience=Auth.AUTH0_AUDIENCE)
...
def logout():
    ...
    return redirect(flask_portal.auth.auth0.api_base_url + '/v2/logout?' + urlencode(params))
...
def dashboard():
    ...
    filtered_apps_endpoints = [(ep,acl,name) for (ep,acl,name) in flask_portal.all_apps_endpoints if acl in access_level]
