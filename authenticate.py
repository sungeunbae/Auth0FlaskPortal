...
#     return response
    ...
    def __fetch_mgmnt_api_token():
        __session = OAuth2Session(Auth.AUTH0_CLIENT_ID,Auth.AUTH0_CLIENT_SECRET) #need a new session to get the token for Mgmnt API
        token = __session.fetch_access_token(Auth.AUTH0_BASE_URL + '/oauth/token', grant_type='client_credentials', audience=Auth.AUTH0_BASE_URL +'/api/v2/')
    ...
    #    print(jwt.get_unverified_claims(token), file=sys.stdout)
        ...
        if rsa_key:
            try:
                ...
                payload = jwt.decode(token, rsa_key, algorithms=[constants.JWT_ALGORITHM], issuer=Auth.ISSUER, audience=Auth.AUTH0_AUDIENCE, options = constants.JWT_VERIFY_DEFAULTS)
